    ...
    def unreal_import(self, episode_name, output_files, shot_cache_task, completed_status, task_type):
        ...
        for of in output_files:
            ...
            if of["name"].lower().endswith(".zip") or of["name"].lower().endswith("7z"):
                ...
                source_file_path = os.path.normpath(source_file_path.replace("/mnt/content/productions", "Z://productions"))    

                if not of["name"] in source_file_path:
                    source_file_path = os.path.join(source_file_path, of["name"])

                source_path = os.path.normpath(source_file_path.replace("/mnt/content/productions", "Z://productions"))
                target_path = os.path.normpath(source_file_path.replace("Z:\\productions", self.swing_root))
                ...
                if extract_archive(SwingSettings.get_instance().bin_7z(), source_path, cache_dir):
                    try:
                        ...
                        try:
                            ...
                            try:
                                abc_cache_dir = os.path.join(cache_dir, "cache", "abc")

                                if not os.path.exists(abc_cache_dir):
                                    abc_cache_dir = os.path.join(cache_dir, "abc")
