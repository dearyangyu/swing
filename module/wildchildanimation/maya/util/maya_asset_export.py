...
class MayaAssetHandler(SwingMaya):
    ...
    def get_fbx_set(self):
        ...
        fbx_set_A = cmds.ls('*:*_fbx_set', type = 'objectSet')
        fbx_set_B = cmds.ls('*:*:*_fbx_set', type = 'objectSet')
        fbx_set_C = cmds.ls('*:*:*:*_fbx_set', type = 'objectSet')
        for set in fbx_set_A:
            fbx_set.append(set)
        for set in fbx_set_B:
            fbx_set.append(set)
        for set in fbx_set_C:
            fbx_set.append(set)
    ...
    def smooth_meshes(self, meshes):
        for mesh in meshes:
                shapenode = cmds.listRelatives(mesh, ni = True, shapes=True )
                if shapenode:
    ...
    def export_static_fbx(self, polysmooth, do_combine, set_to_origin, parent_rotation,export_collection) :
        ...
        for prop in export_collection_longname:
            ...
            parent = self.find_parent(prop, export_collection)
            
            if parent == 'root':
                pass
            else:
                cmds.setAttr('{}.rotateX'.format(parent), 0)
                cmds.setAttr('{}.rotateY'.format(parent), 0)
                cmds.setAttr('{}.rotateZ'.format(parent), 0)
            ...
            export_command = ('FBXExport -f ("' + str(path) + str(export_name) + '.fbx") -s;')
            mel.eval(export_command)
            #if parent_rotation[3] == 'root':
            #    pass
            #else:
            print('{}.rotateX'.format(parent_rotation[3]), parent_rotation[0])
            print('{}.rotateY'.format(parent_rotation[3]), parent_rotation[1])
            print('{}.rotateZ'.format(parent_rotation[3]), parent_rotation[2])
            cmds.setAttr('{}.rotateX'.format(parent_rotation[3]), parent_rotation[0])
            cmds.setAttr('{}.rotateY'.format(parent_rotation[3]), parent_rotation[1])
            cmds.setAttr('{}.rotateZ'.format(parent_rotation[3]), parent_rotation[2])


    # getting attributes to write to json
    def get_prop_attr(self, export_props, sys_path, local_path, export_type):
        asset_name = self.get_asset_name()

        count = 0
        prop_dictionary = []
        prop_dictionary.append({'asset_type': export_type, 'game_path': '/Game/assets/environments/' + asset_name + '/components/', 'local_path': local_path, 'shared_path': sys_path})
        for prop in export_props:
            namespace = self.get_geo_name(prop, "namespace")
            prop_name = self.get_geo_name(prop, "prop")
            actor_name = self.get_geo_name(prop, "actor")
            ...
            print('parent :{}'.format(curr_parent))   
            print(parent_rx)
            print(parent_ry)
            print(parent_rz)
            ...
            if len(namespace) > 0:
                dictionary = {'name':('sm_' + prop_name), 'tx':tx, 'ty':ty, 'tz':tz, 'rx':(rx + parent_rx), 'ry':(ry + parent_ry), 'rz':(rz + parent_rz), 'sx':sx, 'sy':sy, 'sz':sz, 'fullname':(prop), 'parent':prop_parent, 'actor_name':actor_name}
            else:
        ...
        return(prop_dictionary, parent_rx, parent_ry, parent_rz , curr_parent)

    def get_rig_attr(self, export_assets, sys_path, local_path, export_type):
        asset_name = self.get_asset_name()
            
        rig_dictionary = []
        rig_dictionary.append({'asset_type': export_type, 'game_path': '/Game/assets/characters/' + asset_name + '/components/fbx/', 'local_path': local_path, 'shared_path': sys_path})
        
        for asset in export_assets:
            actor_name = self.get_geo_name(asset_name, "actor")

            dictionary = ({'asset_name': 'sk_{}'.format(asset_name),'fullname':(export_assets), 'skeleton_name':'sk_{}_Skeleton'.format(asset_name)})
        
        rig_dictionary.append(dictionary)
    ...
    def write_json_file(self, export_type, export_assets):
        ...
        self.log_output("Wheelbarrow: exporting {}, json: {}, path: {}".format(self.get_scene_name(), json_filename, self.get_scene_path()))
        ...
        with open(json_dir + json_filename, 'w') as json_file:
            if export_type == "env":
                asset_dictionary = self.get_prop_attr(export_assets, shared_path, json_dir, export_type)

            if export_type == "rig":
                asset_dictionary = self.get_rig_attr(export_assets, shared_path, json_dir, export_type)

            json.dump(asset_dictionary[0],json_file , indent=4)
            return(asset_dictionary[1], asset_dictionary[2], asset_dictionary[3], asset_dictionary[4])
    ...
    def export_selected_staticmesh_env(self):
        ...
        parent_rotation = self.write_json_file("env", export_collection)
        #self.breakAnimCurveConnections(export_collection)
        self.export_static_fbx(False, True, True, parent_rotation, export_collection)
