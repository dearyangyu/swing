...
def export_skeletal_fbx(polysmooth, export_set):
    
    skel_fbx_export_settings()
...
def get_geo_name(prop, prepostfix):
...
def export_static_fbx(polysmooth, do_combine, set_to_origin) :
    ...
    for prop in export_collection_longname:
        ...
        asset_name = get_geo_name(prop, "prop")
...
def get_prop_attr(export_props):
    ...
    for prop in export_props:
        namespace = get_geo_name(prop, "namespace")
        prop_name = get_geo_name(prop, "prop")
        ...
        if len(namespace) > 0:
            dictionary = {'name':('sm_' + prop_name), 'tx':tx, 'ty':ty, 'tz':tz, 'rx':rx, 'ry':ry, 'rz':rz, 'sx':sx, 'sy':sy, 'sz':sz, 'fullname':(prop + '_' + namespace), 'parent':prop_parent}
        else:
            dictionary = {'name':('sm_' + prop_name), 'tx':tx, 'ty':ty, 'tz':tz, 'rx':rx, 'ry':ry, 'rz':rz, 'sx':sx, 'sy':sy, 'sz':sz, 'fullname':(prop), 'parent':prop_parent}
        prop_dictionary.append(dictionary)
        
    return(prop_dictionary)

def get_rig_attr(export_assets):
    asset_name = get_asset_name()
...
def write_json_file(export_type, export_assets):
    json_filename = (get_scene_name() + ".json") 
    print("Hurlie: exporting {}, json: {}, path: {}".format(get_scene_name(), json_filename, get_scene_path()))
    ...
    with open(json_dir + json_filename, 'w') as json_file:
        if export_type == "env":
            asset_dictionary = get_prop_attr(export_assets)

        if export_type == "rig":
            asset_dictionary = get_rig_attr(export_assets)
...
def export_selected_staticmesh_env():
    ...
    write_json_file("env", export_collection)
...
def export_rig_chr():
    ...
    for ri_set in ri_sets:
        ...
        write_json_file("rig", export_collection)
        export_skeletal_fbx(True, ri_set)