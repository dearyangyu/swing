...
'''
frame in for sc010_sh010 is 1125 and frame out is 1797 in premiere (total length is 673 frames)
frame in for sc010_sh020 is 1798 and frame out is 1951 in premiere (total length is 154 frames)
12:25
not sure why the screengrab you sent has them all 1 frame shorter?
...
def _build_shot(item, track_no, track_range, existing_shot=None):
    if _DEBUG:
        print("_build_shot Track #{} Range {}".format(track_no, track_range))
    ...
    if _DEBUG:
        print("Adding shot {}".format(item_name))

    camera = None
    if existing_shot is None:
        camera = cmds.camera(name=item_name.split('.')[0] + '_cam')[0]
    '''        
    cmds.shot(
        existing_shot or item_name.split('.')[0],
    ...
    cmds.shot(
        ...
        sequenceStartTime=track_range.start_time.value + 1,
        sequenceEndTime=track_range.end_time_exclusive().value
    )   


def _build_track(track, track_no, existing_shots=None):
    if _DEBUG:
        print("_build_track Track #{} Length {}".format(track_no, len(track)))
...
def build_sequence(timeline, clean=False):
    if _DEBUG:
        print("build_sequence {}".format(timeline))
    ...
    for track_no, track in enumerate(reversed(tracks)):
        if len(track) > 0:
            ...
            _build_track(track, track_index, existing_shots=existing_shots)
            track_index += 1
        else:
            print("build_sequence skipping empty track {}".format(track_no))
...
def _get_gap(duration):
    if _DEBUG:
        print("_get_gap({})".format(duration))
